...
const text = {
  en: {
    ...
    writing: 'Articles in System Block', writingSub: 'Guides and essays on language, literature, and AI. All articles are in Russian.',
    guides: [
      ...
      ['Exporting Telegram channels with Python', 'Tutorial · August 2025', 'https://sysblok.ru/courses/kak-skachat-ves-telegram-avtomatizacija-vygruzki-kanalov-s-pomoshhju-python/', 'How to collect posts automatically and discover linked Telegram channels for media research.'],
      ['13 questions about large language models', 'Explainer · January 2026 · Co-authored', 'https://sysblok.ru/ai/llm/', 'Why language models can write code, struggle to count letters, and sometimes produce unexpected answers.'],
      ['The scriptor is dead, long live the author!', 'Essay · December 2025', 'https://sysblok.ru/blog/blog_dmitrii_pronin/skriptor-umer-da-zdravstvuet-avtor-est-li-mesto-chelovecheskomu-tekstu-v-jepohu-bolshih-jazykovyh-modelej/', 'Why a recognisable human voice matters amid the growing flood of AI-generated writing.'],
    ],
  ...
  ru: {
    ...
    writing: 'Статьи в журнале Системный Блокъ', writingSub: 'О языке, литературе и нейросетях. Гайды и эссе.',
    guides: [
      ...
      ['Как скачать весь Telegram: выгрузка каналов с Python', 'Руководство · август 2025', 'https://sysblok.ru/courses/kak-skachat-ves-telegram-avtomatizacija-vygruzki-kanalov-s-pomoshhju-python/', 'Как автоматически собирать посты и находить связанные Telegram-каналы для исследования медиасреды.'],
      ['13 стыдных вопросов про LLM', 'Объясняющий материал · январь 2026 · в соавторстве', 'https://sysblok.ru/ai/llm/', 'Почему языковые модели пишут код, ошибаются в подсчёте букв и порой дают неожиданные ответы.'],
      ['Скриптор умер, да здравствует автор!', 'Эссе · декабрь 2025', 'https://sysblok.ru/blog/blog_dmitrii_pronin/skriptor-umer-da-zdravstvuet-avtor-est-li-mesto-chelovecheskomu-tekstu-v-jepohu-bolshih-jazykovyh-modelej/', 'Почему в потоке нейросетевых текстов нам всё важнее слышать живой авторский голос.'],
    ],
...
function gwasBrief(lang,c) {
  return `<section id="brief"><h2>${c.briefTitle}</h2>${lang==='ru'?`
    ...
    <p>Частотный профиль показывает, как часто в тексте встречается каждое слово. В нашей аналогии он играет роль ДНК, а авторство соответствует фенотипу. Мы хотим понять, какие особенности этого профиля связаны с конкретным писателем.</p>
    <p>Возьмём слово «чувствовать». Посчитаем его частоту во фрагментах Толстого и других авторов, приведём значения к общему масштабу и проверим, есть ли связь с авторством. Так же генетики проверяют связь отдельного маркера с изучаемым признаком. Эту процедуру повторяем для всех слов.</p>
    <p>В нашем исследовании более высокая частота «чувствовать» оказалась связана с авторством Толстого. Связь сохранила статистическую значимость после поправки на множество проверок. Собрав такие результаты для разных слов, мы получаем подробный портрет авторской речи и можем вернуться к текстам, чтобы понять, за счёт чего он складывается.</p></details>
  ...
  `:`
    ...
    <p>A frequency profile records how often each word occurs in a text. In our analogy, this profile plays the role of DNA, while authorship is the phenotype. We want to find out which features of the profile are associated with a particular writer.</p>
    <p>Take «чувствовать» (to feel). We measure its frequency in passages by Tolstoy and other authors, put the values on a common scale, and test for an association with authorship. Geneticists follow a similar procedure when testing a marker against a trait. We repeat this process for every word.</p>
    <p>In our study, a higher frequency of «чувствовать» was associated with Tolstoy’s authorship. The association remained statistically significant after correction for multiple testing. Bringing these findings together gives us a detailed picture of a writer’s language, which we can explore further by returning to the texts.</p></details>
